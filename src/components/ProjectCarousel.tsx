...
const initialFocusedProject = (projects: ProjectDetailsProps[]) => projects.find(p => p.name === "ANNIverse") as ProjectDetailsProps;
...
const ProjectCarousel: React.FC<ProjectCarouselProps> = ({ projects = [] }) => {
  const initial = initialFocusedProject(projects);
  const [focusedProjectName, setFocusedProjectName] = useState<string>(initial?.name);
  ...
  const { focusedProject, otherProjects } = projects.reduce((obj, project) => {
    ...
    return obj;
  }, { focusedProject: initial ?? {}, otherProjects: [] } as { focusedProject: ProjectDetailsProps, otherProjects: ProjectDetailsProps[] })
